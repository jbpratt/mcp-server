...
func TestListTasks(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			prefix:    "",
			input: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1", Namespace: "test-namespace1"}},
				{ObjectMeta: metav1.ObjectMeta{Name: "task2", Namespace: "test-namespace2"}},
			},
			output: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1", Namespace: "test-namespace1"}},
			},
		},
		{
			...
			prefix:    "",
			input: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1", Labels: map[string]string{"key": "value"}}},
				{ObjectMeta: metav1.ObjectMeta{Name: "task2"}},
			},
			output: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1", Labels: map[string]string{"key": "value"}}},
		...
		},
		{
			...
			input: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1"}},
				{ObjectMeta: metav1.ObjectMeta{Name: "othertask"}},
			},
			output: []*v1.Task{
				{ObjectMeta: metav1.ObjectMeta{Name: "task1"}},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fakeClient := fakepipelineclientset.NewSimpleClientset()
			informer := informers.NewSharedInformerFactory(fakeClient, 0)
			taskInformer := informer.Tekton().V1().Tasks()

			for _, item := range tt.input {
				_, _ = fakeClient.TektonV1().Tasks(item.Namespace).Create(t.Context(), item, metav1.CreateOptions{})
				_ = taskInformer.Informer().GetIndexer().Add(item)
			}

			stopCh := make(chan struct{})
			defer close(stopCh)
			informer.Start(stopCh)
			cache.WaitForCacheSync(stopCh, taskInformer.Informer().HasSynced)

			actual, err := listTasksHelper(taskInformer, tt.namespace, tt.lselector, tt.prefix)
			require.NoError(t, err)

			var actualObjs []*v1.Task
			_ = json.Unmarshal([]byte(actual), &actualObjs)
			require.ElementsMatch(t, tt.output, actualObjs)
		})
	}
...
func TestListPipelines(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: []*v1.Pipeline{
				...
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline2"}},
			},
			output: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1"}},
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline2"}},
		...
		},
		{
			...
			prefix:    "",
			input: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1", Namespace: "test-namespace1"}},
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline2", Namespace: "test-namespace2"}},
			},
			output: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1", Namespace: "test-namespace1"}},
			},
		},
		{
			...
			prefix:    "",
			input: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1", Labels: map[string]string{"key": "value"}}},
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline2"}},
			},
			output: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1", Labels: map[string]string{"key": "value"}}},
		...
		},
		{
			...
			input: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1"}},
				{ObjectMeta: metav1.ObjectMeta{Name: "otherpipeline"}},
			},
			output: []*v1.Pipeline{
				{ObjectMeta: metav1.ObjectMeta{Name: "pipeline1"}},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fakeClient := fakepipelineclientset.NewSimpleClientset()
			informer := informers.NewSharedInformerFactory(fakeClient, 0)
			pipelineInformer := informer.Tekton().V1().Pipelines()

			for _, item := range tt.input {
				_, _ = fakeClient.TektonV1().Pipelines(item.Namespace).Create(t.Context(), item, metav1.CreateOptions{})
				_ = pipelineInformer.Informer().GetIndexer().Add(item)
			}

			stopCh := make(chan struct{})
			defer close(stopCh)
			informer.Start(stopCh)
			cache.WaitForCacheSync(stopCh, pipelineInformer.Informer().HasSynced)

			actual, err := listPipelinesHelper(pipelineInformer, tt.namespace, tt.lselector, tt.prefix)
			require.NoError(t, err)

			var actualObjs []*v1.Pipeline
			_ = json.Unmarshal([]byte(actual), &actualObjs)
			require.ElementsMatch(t, tt.output, actualObjs)
		})
	}
